...
from image import image_type_detection, compress_image_by_ziporxy, get_image_info, convert_webp_to_png, ziprxoy_zip,compute_webp_ssim
...
def main():
    ...
    image_output_file = os.path.join(config['output_dir'],
                                     datetime.now().strftime("%Y%m%d%H%M%S") + "_ziproxy_" + config['image_output_file'])
        ...
        for line in r_handler:
            try:
                if line and line.strip():
                    ...
                    compress_md5_75, compress_size_75, ziprxpy_run_time_75 = compress_image_by_ziporxy(reponse_body)
                    w_handler.write("{}\t{}\t{}\t{}\t{}\n".format(base_str, image_model,compress_md5_75, compress_size_75, ziprxpy_run_time_75))
